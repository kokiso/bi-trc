      ...
      if ($rotina == "biSimples") {
        ...
        if ($lote[0]->qualSelect == "bisVeiculoUni") {
          ...
          switch ($lote[0]->levpes) {
            case "LP":
              $frota = " AND (A.VCL_FROTA IN('L','P'))";
              break;
            case "L":
              $frota = " AND (A.VCL_FROTA='L')";
              break;
            case "P":
              $frota = " AND (A.VCL_FROTA='P')";
              break;
          }
          $sql .= $gpo;
          if ($lote[0]->coduni > 0) {
        ...
        if ($retCls['retorno'] != "OK") {
          ...
          for ($lin = 0; $lin < $tam; $lin++) {
            ...
            array_push(
              $arrJava,
              [
                "ID"         => ($lin + 1), "NOME"       => $tblMtr[$lin]["NOME"], "QTOS"       => $tblMtr[$lin]["QTOS"], "PERCENTUAL" => number_format($pc, 2), "COLCOR"     => $colCor, "COLCLASS"   => $colClass
              ]
            );
          };
        ...
        if ($retCls['retorno'] != "OK") {
          ...
          for ($lin = 0; $lin < $tam; $lin++) {
            ...
            if ($achei == false) {
              $qtosN += $tblAll[$lin]["QTOS"];
              array_push($tblInfN, [
                "SIGLA"       =>  $tblAll[$lin]["ID"], "NOME"       =>  $tblAll[$lin]["NOME"], "QTOS"       =>  $tblAll[$lin]["QTOS"], "PERCENTUAL" => 0, "COLCOR"     => "*", "COLCLASS"   => "*", "GRAFICO"    => (($tblAll[$lin]["ID"]   == "EV")  ? "S" : (($tblAll[$lin]["ID"] == "EVC") ? "S" : (($tblAll[$lin]["ID"] == "FB")  ? "S" : "N")))
              ]);
            };
            //////////////////////
            //     tblInfU      //
            ...
            if ($achei == false) {
              $qtosU += $tblAll[$lin]["QTOS"];
              array_push($tblInfU, [
                "SIGLA"       =>  $tblAll[$lin]["UNIDADE"], "NOME"       =>  $tblAll[$lin]["UNIDADE"], "QTOS"       =>  $tblAll[$lin]["QTOS"], "PERCENTUAL" => 0, "COLCOR"     => "*", "COLCLASS"   => "*", "GRAFICO"    => "N"
              ]);
            };
            //////////////////////
            //     tblInfP      //
            ...
            if ($achei == false) {
              $qtosP += $tblAll[$lin]["QTOS"];
              array_push($tblInfP, [
                "SIGLA"       =>  $tblAll[$lin]["POLO"], "NOME"       =>  $tblAll[$lin]["POLO"], "QTOS"       =>  $tblAll[$lin]["QTOS"], "PERCENTUAL" => 0, "COLCOR"     => "*", "COLCLASS"   => "*", "GRAFICO"    => "N"
              ]);
            };
          };
  ...
  <script>
    ...
    function fncContar(qualSelect, qualSpan, qualCodUni, qualCodPol, qualLevPes, qualCodGpo) {
      ...
      if (retPhp[0].retorno == "OK") {
        ...
        switch (qualSpan) {
          case "qtosKm":
            velocMedia[0] = parseInt(parseFloat(retPhp[0]["dados"][0]));
            break;
          case "qtosHRod":
            velocMedia[1] = parseInt(parseFloat(retPhp[0]["dados"][0]));
            break;
        };
