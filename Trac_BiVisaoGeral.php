      ...
      if($retCls["retorno"] != "OK"){
        ...
        if( $rotina=="biSimples" ){
          ...
          if( $retCls['retorno'] != "OK" ){
            ...
            $retorno='[{"sql":"'.$sql.'","retorno":"OK","dados":'.json_encode($arrJava).',"erro":""}]';
          };  
        };
        ////////////////
        /// INFR. GPO //
        ...
        if( $rotina=="biInfracaoGpo" ){
            ...
            $sql = "SELECT TOP 10 COUNT(INF.ID) AS INFRACOES, GPO_NOME AS NOME FROM VEICULO V
                         INNER JOIN INFRACAO INF ON V.VCL_CODIGO = INF.PLACA AND INF.ANO_MES =".$codmes."
                         LEFT OUTER JOIN GRUPOOPERACIONAL GPO ON V.VCL_CODGPO = GPO.GPO_CODIGO
                         LEFT OUTER JOIN UNIDADE UNI ON V.VCL_CODUNI=UNI.UNI_CODIGO
                         LEFT OUTER JOIN USUARIOUNIDADE UU ON V.VCL_CODUNI=UU.UU_CODUNI AND UU.UU_CODUSR =".$_SESSION['usr_codigo']."
                WHERE VCL_CODGPO IS NOT NULL";
            $sql.=$frota;
            $sql.=" GROUP BY VCL_CODGPO, GPO_NOME 
  ...
  <head>
    ...
    <script>
      ...
      function iniciarBi(ibCodUni,ibDesUni,ibCodPol,ibDesPol){
        ...
        //////////////////////////////////////////////////////////////
        fncFiltrarTableInfracao("infracao"        ,"tblinfN"  ,"divInfracaoN"  ,"qtosInfracaoN" ,pubCodUni,pubCodPol,pubLevPes);
        fncFiltrarGraficoGrupoOperacional("infracao"        ,"tblinfN"  ,"divInfracaoN"  ,"qtosInfracaoN" ,pubCodUni,pubCodPol,pubLevPes);
