...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (document.getElementsByTagName('verGrafico') != undefined) {
        arrFill = ["rgba(143,188,143,0.5)", "rgba(151,187,205,0.5)", "rgba(0,206,209,0.5)", "rgba(160,82,45,0.5)", "rgba(205,133,63,0.5)", "rgba(46,139,87,0.5)", "rgba(100,149,237,0.5)", "rgba(95,158,160,0.5)", "rgba(205,92,92,0.5)", "rgba(143,188,143,0.5)", "rgba(151,187,205,0.5)"];
        arrBorder = ["rgba(143,188,143,1)", "rgba(151,187,205,1)", "rgba(0,206,209,1)", "rgba(160,82,45,1)", "rgba(205,133,63,1)", "rgba(46,139,87,1)", "rgba(100,149,237,1)", "rgba(95,158,160,1)", "rgba(205,92,92,1)", "rgba(143,188,143,1)", "rgba(151,187,205,1)"];
    };
    /*
...
function mascaraGerencial(campoData, campoEvento) {
    if ((campoEvento.keyCode >= 48) && (campoEvento.keyCode <= 57)) {
        var data = campoData.value;
        if ((data.length == 1) || (data.length == 4) || (data.length == 7) || (data.length == 10)) {
            data = data + '.';
            campoData.value = data;
            return true;
        };
...
function retornarZIndex() {
    ...
    var maior = 0;
    for (var i = 0; i < di; i++) {
        if (maior <= divs[i].style.zIndex)
            maior = (divs[i].style.zIndex + 1);
    };
...
function criarTD(coluna, arrTD) {
    ...
    if (arrTD[1] != 'NAOCLASS')
        ceTd.setAttribute('class', arrTD[1]);
    ...
    switch (arrTD[2]) {
        case '0':
            break;
        case '1':
            if (cmp.floatNA(arrTD[0]) < 0)
                ceTd.classList.add('corVermelho');
            break;
        ...
        case '2':
            ...
            divTd.setAttribute('onclick', 'copiaDocumento(this.parentNode.parentNode.cells[' + arrTD[0] + '].innerHTML);');
            ceTd.appendChild(divTd);
            break;
        case '3':
            if (cmp.floatNA(arrTD[0]) < 0)
                ceTd.classList.add('fontVermelho');
            break;
        case '4':
...
function tableJson(tbl) {
    ...
    el = document.getElementById(tbl).getElementsByTagName('thead')[0].getElementsByTagName('th');
    nc = el.length;
...
HTMLElement.prototype.insereRodape = function(rotina, cadastro) {
    if (rotina == '') {
        this.innerHTML = 'Usuário:' + jsPub[0].DESUSU + '  |  Empresa:' + jsPub[0].EMP_APELIDO;
    } else {
        var ceBut, ceImg;
        var ceTable = document.createElement("table");
        ceTable.style.background = "-webkit-gradient(linear, left top, left bottom, color-stop(0%, #3093c7), color-stop(100%, #1c5a85))";
        ceTable.color = "white";
            ...
            for (var lin = 0; lin < tam; lin++) {
                ...
                switch (cadastro[lin]) {
                    case "SB_TCliente2017.php":
                        ...
                        ceBut.type = "button";
                        ceBut.id = "cadCli";
                        ceBut.title = "Ajuda para consulta/manutenção em clientes/favorecidos";
...
HTMLElement.prototype.insereItensMo = function(arr) {
    ...
    for (var lin = 0; lin < cont; lin++) {
        ...
        switch (arr[lin]) {
            case 'Ajuda':
                ceLi.addEventListener('click', function() { fAjuda(); });
                strCtx = ' Ajuda';
                strImg = "fa fa-info";
                break;
                /*  
...
function clsMensagem(cabec) {
    ...
    this.dataCompara = function(campo, dat1, dat2, condicao) {
        ...
        switch (condicao) {
            case 'dataMaior':
                if (d1 <= d2) this.strLista += '<tr><td>CAMPO <b>' + campo + '</b> ' + dat1 + ' DEVE SER MAIOR QUE ' + dat2 + '</td></tr>';
                break;
            case 'dataMaiorIgual':
                if (d1 < d2) this.strLista += '<tr><td>CAMPO <b>' + campo + '</b> ' + dat1 + ' DEVE SER MAIOR OU IGUAL ' + dat2 + '</td></tr>';
                break;
            case 'dataDiferente':
                if (d1 == d2) this.strLista += '<tr><td>CAMPO <b>' + campo + '</b> ' + dat1 + ' DEVE SER DIFERENTE DE ' + dat2 + '</td></tr>';
                break;
            case 'dataMenorIgual':
                if (d1 > d2) this.strLista += '<tr><td>CAMPO <b>' + campo + '</b> ' + dat1 + ' DEVE SER MENOR OU IGUAL ' + dat2 + '</td></tr>';
                break;
        };
    };
    //
...
function cxDialogo(js) {
    ...
    js.campos.forEach(function(c) {
         ...
         */
        if (objinpt == "input")
            objImg = (tagValida(c.imagem) ? c.imagem : 'fa-info');
        ...
        if (c.type == "integer")
            ceInp.onkeypress = function() { return mascaraInteiro(event) };
        if (c.type == "date")
            ceInp.onkeyup = function() { return mascaraData(this, event) };
...
function segundosRev(parStr) {
    ...
    var ret = 0;
    if (splt.length == 4)
        return ((parseInt(splt[0]) * 86400) + (parseInt(splt[1]) * 3600) + (parseInt(splt[2]) * 60) + parseInt(splt[0]));
    if (splt.length == 4)
        return ((parseInt(splt[1]) * 3600) + (parseInt(splt[2]) * 60) + parseInt(splt[0]));
}
