<?php
  session_start();
  if( isset($_POST["grdIdentificacaoMot"]) ){
    try{     
      ...
      $vldr     = new validaJSon();          
      $retorno  = "";
      $retCls   = $vldr->validarJs($_POST["grdIdentificacaoMot"]);
      ///////////////////////////////////////////////////////////////////////
      ...
      if($retCls["retorno"] != "OK"){
        ...
        if( $rotina=="select" ){
          ...
          $sql =" SELECT MVM.MVM_POSICAO AS ID";
					$sql.="      ,MVM.MVM_PLACA AS PLACA";
					$sql.="      ,MTR.MTR_NOME AS MOTORISTA";
          $sql.="      ,MVM.MVM_RFID AS RFID";
					//$sql.="      ,MVM.MVM_DATAGPS AS DATAGPS";
					$sql.="  		 ,CONVERT(VARCHAR(23),MVM.MVM_DATAGPS,127) AS DATAGPS";
					$sql.="      ,MVM.MVM_LOCALIZACAO AS ONDE";
					$sql.="      ,MVM.MVM_TURNO AS TURNO";
					$sql.="      ,UNI.UNI_NOME AS UNIDADE";
					$sql.="	     ,POL.POL_NOME AS POLO";
					$sql.="      ,MVM.MVM_IGNICAO AS IGNICAO";
					$sql.="  FROM MOVIMENTO AS MVM";
					$sql.="  LEFT OUTER JOIN MOTORISTA AS MTR ON MVM.MVM_CODMTR = MTR.MTR_CODIGO";
					$sql.="  LEFT OUTER JOIN UNIDADE AS UNI ON MVM.MVM_CODUNI = UNI.UNI_CODIGO";
					$sql.="  LEFT OUTER JOIN POLO AS POL ON MVM.MVM_CODPOL = POL.POL_CODIGO";
					$sql.="  LEFT OUTER JOIN USUARIOUNIDADE UU ON MVM_CODUNI=UU.UU_CODUNI AND UU.UU_CODUSR=".$lote[0]->codusu;
					$sql.=" WHERE ( MVM.MVM_POSICAO BETWEEN ".$bwI." AND ".$bwF.")";
					$sql.="  AND (MVM.MVM_RFID <> '-')";
					$sql.="  AND (MTR.MTR_NOME='NAO INFORMADO')";
					$sql.="  AND ((MVM.MVM_CODEVESS = '7036') OR (MVM.MVM_CODEVESS = '164'))";
					$sql.="  AND (MVM.MVM_IGNICAO = '1')";
...
<!DOCTYPE html>
  <head>
    ...
    <title>Identificação/motorista</title>
    ...
    <script>
      ...
      document.addEventListener("DOMContentLoaded", function(){ 
        ...
        jsBi={
          ...
          ,"relTitulo"      : "BI - Identificação Motorista"                      // Titulo do relatório
      ...
      function btnFiltrarClick() { 
        ...
        fd = new FormData();
        fd.append("grdIdentificacaoMot" , clsJs.fim());
        msg     = requestPedido("Trac_grdIdentificacaoMot.php",fd); 
        ...
        if( retPhp[0].retorno == "OK" ){
          ...
          retPhp.forEach(function(lin){
            retJs.push([
              ...
              ,lin.CODUNI
              ,lin.CODMTR
              ,(lin.MOTORISTA).substr(0,40)
              ,segundosEm(lin.RODANDO,"ddhms" )
              ,segundosEm(lin.PARADO,"ddhms" )
              ,segundosEm( (lin.RODANDO+lin.PARADO),"ddhms" )
              ,jsNmrs(lin.TOTALKM).dec(4).real().ret()
              ,jsNmrs( (lin.TOTALKM/(lin.RODANDO/3600)) ).dec(2).real().ret() // 
              ,lin.UNIDADE
              ,lin.POLO
            ...
            pubKm       += parseFloat(lin.TOTALKM);
          });
          jsBi.registros=objBi.addIdUnico(retJs);
          jsBi.relTitulo="BI Produtividade por motorista em "+document.getElementById("cbIni").value;
          jsBi.indiceTable="MOTORISTA";
          objBi.ordenaJSon(jsBi.indiceTable,false);  
          objBi.montarBody2017();
					*/
        };  
      ...
      function biDetalheClick(){
        ...
        try{        
          ...
          if( chkds[0].MES==0 ){
            ...
            fd          = new FormData();
            fd.append("grdIdentificacaoMot" , clsJs.fim()); 
            var req = requestPedido("Trac_grdIdentificacaoMot.php",fd);
      ...
      function biImprimirClick(){
        ...
        var imprimir = '{'
          ...
          + ',"imprimir":'
          + '[{"SetFont":["Arial","B",8]}'          
          + ',{"SetFillColor":["cinzaclaro","8","220"]}'
          + ',{"Cell":[205,8,"RELATORIO:'+jsBi.relTitulo+'",0,0,"C"]}'
          + ',{"Ln":[8]}'
          + ',{"SetFillColor":["branco","5","220"]}'                    
          + ',{"Cell":[75,6,"MOTORISTA"  ,1,0,"L"]}'
        ...
        for(var fc=0; fc<tamC;fc++){
          ...
          locKm       =  jsNmrs(retPhp[fc].TOTALKM).dec(4).dolar().ret();
          /////////////////////////////////
        ...
        imprimir +=
         ',{"Ln":[2]}'
        +',{"SetFont":["Arial","B",8]}'
        + ',{"SetFillColor":["cinzaclaro","6","220"]}'
        +',{"Cell":[73,6,"TOTAL",0,0,"L"]}' 
